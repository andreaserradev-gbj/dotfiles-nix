...
1. **`git add` before any `--flake` command.** Flakes only see git-tracked
   files; an untracked new module is invisible to evaluation, and the error Nix
   prints for it is `Path 'X' … is not tracked by Git` with the `git add` that
   fixes it.
...
3. **Blast radius: `modules/` moves every host; `hosts/<host>/` moves one.**
   ...
   cannot name is the failure. `hplaptop` is updated by her user running
   `nrb`, so she must not move unless you meant her.
...
5. **Verify on `main`**: the squash push re-runs CI, and `advance-verified`
   fast-forwards `verified` — the ref hplaptop's `nrb --refresh` pulls
   ([doc/workflow.md](doc/workflow.md), `modules/home/maintenance.nix`), so
   Elisa installs only commits CI has already built green. geekom is rebuilt
   by hand from `main`. While `main` is red, `verified` stops advancing and
   the laptop silently stops receiving updates.

## Task routing

The [README's documentation index](README.md#documentation-index) maps each
topic to the doc that owns it — rebuilds and release upgrades
([doc/workflow.md](doc/workflow.md)), installs ([doc/install-vm.md](doc/install-vm.md)
plus the two bare-metal docs), dev shells
([doc/dev-environments.md](doc/dev-environments.md)), adopting a tool
([doc/adopting-tools.md](doc/adopting-tools.md)), the VM console
([doc/vm-console.md](doc/vm-console.md)), and gotchas
...
5. **Push issued?** A commit whose CI fails cannot reach hplaptop, but fix or
   revert it promptly — see "Release cycle" above, step 5.
